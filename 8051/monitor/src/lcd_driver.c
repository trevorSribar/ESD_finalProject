...
void lcd_init(){
    ...
    lcd_writeAddress(LCD_SOFT_RESET_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    lcd_initdelay(LCD_DELAY_120_MS);

    // SLEEP OUT
    lcd_writeAddress(LCD_SLEEP_OUT_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    lcd_initdelay(LCD_DELAY_5_MS);

    // Interface Pixel Format
    lcd_writeAddress(LCD_INTERFACE_PIXEL_FORMAT_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_16_BITS_PER_PIXEL_MODE|LCD_ENSURE_WRITE_ADDRESS);

    // Memory Access Control
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_MEMORY_ACCESS_CONTROL_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    ...
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_NORMAL_DISPLAY_MODE_ON|LCD_ENSURE_WRITE_ADDRESS);

    // Comlumn Address Set
    lcd_writeAddress(LCD_COLUMN_SET_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    LCD_COMMAND_REGISTER_PIN = 1;
    lcd_writeAddress(0); // high byte
    lcd_writeAddress(0); // low byte
    lcd_writeAddress((LCD_PIXEL_HRES-1)>>8|LCD_ENSURE_WRITE_ADDRESS); // high byte
    lcd_writeAddress((LCD_PIXEL_HRES)&0xFF|LCD_ENSURE_WRITE_ADDRESS); // low byte

    // Page Address Set
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_PAGE_SET_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    LCD_COMMAND_REGISTER_PIN = 1;
    lcd_writeAddress(0); //high byte
    lcd_writeAddress(0); //low byte
    lcd_writeAddress((LCD_PIXEL_VRES-1)>>8|LCD_ENSURE_WRITE_ADDRESS); // high byte
    lcd_writeAddress((LCD_PIXEL_VRES)&0xFF|LCD_ENSURE_WRITE_ADDRESS); // low byte

    // Display ON
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_DISPLAY_ON|LCD_ENSURE_WRITE_ADDRESS);

    // change the display brightness
    lcd_writeAddress(LCD_DISPLAY_BRIGHTNESS_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
    LCD_COMMAND_REGISTER_PIN = 1;
    lcd_writeAddress(LCD_DISPLAY_BRIGHTNESS_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);

    // Prepare for pixel writes (Start write function)
    LCD_COMMAND_REGISTER_PIN = 0;
    lcd_writeAddress(LCD_MEMORY_WRITE_ADDRESS|LCD_ENSURE_WRITE_ADDRESS);
